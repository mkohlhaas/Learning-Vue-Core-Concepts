<template>
  <div>
    ...
    <div v-if="user.address">
      <label for="age"
        >City:
        <input v-model="user.address.city" placeholder="Enter your city" id="age" />
      </label>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  watch: {
    // observe changes only in user.name property
    'user.name': {
      handler(newValue: string, oldValue: string) {
        console.log({ newValue, oldValue })
      }
    },
    // observe changes in all user properties (deep = true)
    user: {
